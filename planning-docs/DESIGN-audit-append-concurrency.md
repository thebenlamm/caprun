...
One `BEGIN IMMEDIATE`, expressed as rusqlite `TransactionBehavior::Immediate`, wraps the durable-head read, the `events` `INSERT`, and the `chain_anchor` upsert. The write lock is acquired before parent selection. Because `parent_id` is serialized into the payload covered by the event MAC, selecting it outside the transaction permits it to become stale between selection and insertion.

**Authorised implementation amendment (2026-08-05):** `append_event` retains its public `&Connection` signature, so its autocommit branch uses `Transaction::new_unchecked(conn, TransactionBehavior::Immediate)`. In rusqlite 0.32.1, `Connection::transaction_with_behavior` requires `&mut Connection` and therefore cannot satisfy that locked signature. This is not permission to nest transactions: the explicit `is_autocommit` branch below remains mandatory. The three original enclosing sites, which do own mutable connections, continue to use `transaction_with_behavior(TransactionBehavior::Immediate)` as originally pinned.

**Authorised implementation amendment (2026-08-09, CR-01 / Phase 51.1):** `record_github_grant` is the fourth enclosing site: it opens an immediate transaction so the capability row and its authorization event are co-atomic. It is the sole enclosing site using `Transaction::new_unchecked`, because its shared-connection signature is locked by more than twelve call sites and cannot use the mutable-connection constructor. This remains no permission to nest transactions: `append_event` joins that enclosing boundary through the mandatory `is_autocommit` check.
...
| `crates/brokerd/src/audit.rs` | 561 | `record_github_grant` | `in-transaction` | `Phase 51.1 enclosing immediate transaction` |
...
**Completeness result:** 45 production call sites: 2 `session-root`, 39 `chain-continuation`, and 4 `in-transaction`. No production call site outside this table exists. `crates/brokerd/src/policy.rs` also contains the `session_created`/`policy_bound` append pattern at lines 373 and 389, but only inside its `#[cfg(test)] mod tests`; it was mechanically excluded, while the production equivalents are the first two `cli/caprun/src/main.rs` rows. No production append exists in `file_write`'s sibling `file_create`-unrelated modules beyond those listed, and the terminal sink files named here form the adversarial trace checklist for Plan 51-08.
