...
### Collect-then-Block architecture (MANDATED post-research, ruled by opus 2026-07-07)
Research (`12-RESEARCH.md`) found the B1 precedence bug (D-02/D-12a) is not hypothetical — it is the CURRENT code shape. `crates/executor/src/lib.rs`'s per-arg loop returns on the FIRST tainted routing-sensitive arg (early return inside the loop); `SinkBlockedAnchor`/`BlockedPendingConfirmation` (`runtime-core/src/executor_decision.rs`) carry exactly ONE blocked arg by construction. A plan node with both a tainted `to` and a tainted `body` today blocks on `to` only; confirming that shown block silently ships the body's literal too (via `confirmation.rs::confirm()`'s full-snapshot re-invocation). CONTENT-01 makes this reachable for the first time. Opus ruled, after verifying composition with locked decisions:

- **D-14 (MUST, not discretionary):** The executor's per-arg loop changes from first-match-wins to collect-ALL-sensitive-and-tainted-args-then-Block-as-a-set. `ExecutorDecision::BlockedPendingConfirmation`/`SinkBlockedAnchor` become plural (`Vec<BlockedArg>`), each carrying its own literal+taint+provenance_chain.
- **D-15:** I2-over-I1 precedence is PRESERVED unchanged — the collect-all loop still "completes with no Block" in the clean case before Step 0.5's draft-only check runs (matches the recorded v1.2 decision; do not reorder this).
- **D-16:** The EXTRACT-02 unbroken-edge hard gate (raw-read Event → extractor-derived ValueNodes → blocked sink args) and its anti-staple check must hold for EVERY blocked arg in the set, not just one — this is now part of Phase 12's design doc requirement to state explicitly, and Phase 15's implementation/verification scope.
- **D-17:** Single-shot semantics extend to the set: confirm authorizes the WHOLE (sink, {all blocked args}, combined-digest) set, or denies ALL. No partial confirm of a subset.
- **D-18:** This is in-scope hardening, NOT a re-opening of the locked plan-node API — `PlanNode{sink, args: Vec<ValueNode>}` input shape is untouched; only the internal decision/anchor types become plural.
- **D-19:** CONFIRM-03's binding hash MUST cover the FULL SET of blocked args (recipient AND body together as one combined digest) — this is the only shape that satisfies D-02 and D-08 simultaneously without a second confirm round-trip.
- **D-20:** CONFIRM-04's block narration MUST render EVERY blocked arg's literal + provenance, not just the first-match arg — the human must see all tainted values before deciding.
- **D-21:** The adversarial reviewer (D-11) MUST independently re-verify the "`is_content_sensitive` already returns true for email.send's subject/body/attachment" research claim against the actual current source (`crates/executor/src/sink_sensitivity.rs`) — do not let the DESIGN doc rest on the research doc's word. Record explicitly in the DESIGN doc that Phase 14's real work is changing Step 3's *consequence* (no-op → Block), not adding new classification.
- **D-22:** `lettre >=0.11.22` + Mailpit are APPROVED (research's stack recommendation stands). BUT the CRLF/header-injection fixture test (SMTP-05) remains a HARD requirement regardless of lettre's built-in defense — "defends by construction" must be VERIFIED by a passing adversarial fixture in Phase 13, not assumed from the library's reputation alone. The DESIGN doc should specify what that fixture test must assert.
...
- Exact section structure within the DESIGN doc(s), as long as it addresses D-01 through D-22 explicitly and is reviewable against them.
