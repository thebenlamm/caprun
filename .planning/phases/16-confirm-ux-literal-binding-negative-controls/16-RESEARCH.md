...
**Scope recommendation [SUPERSEDED — Round-1 panel ruling, caprun-opus-77]:** This paragraph originally recommended deferring the Allowed-path email.send wiring to Phase 17. That recommendation is RETRACTED. The coordinator's panel traced CONTROL-01 to Phase 16 in REQUIREMENTS.md (:95); Phase 17 only COMPOSES it (ROADMAP:194, REQUIREMENTS:55) and has no plans yet — so deferring left the wiring UNOWNED (the hollowing pattern). Two premises here were also FALSE: (b) the Gate-3 cost — check-invariants.sh already allowlists `server.rs` for the mint tokens, and an email.send Allowed-dispatch branch calling the RESOLVING `sinks::email_smtp` (which resolves, never mints) trips NO gate; and (c) the ROADMAP assignment — Phase 17 "composes," it does not OWN the wiring. **Corrected scope: Phase 16 BUILDS the email.send Allowed-decision dispatch branch (a faithful mirror of the file.create one — the resolve-loop shape at lines 493-508 is exactly right), in Plan 16-01, and Plan 16-03 asserts a REAL trusted clean send lands in Mailpit under scripts/mailpit-verify.sh.** Phase 17 keeps a distinct job: composing CONTROL-01 + the hostile block into ONE run / ONE unbroken DAG (one Uuid, one audit.db, one session).

_(Original deferral text, retained for provenance:)_ Building new Allowed-path email-sink-invocation wiring this phase would (a) require a new dispatch branch in `server.rs` mirroring the `file.create` one, resolving `plan_node.args` directly (there is no `PendingConfirmation` for an Allowed decision — a NEW resolve-loop, structurally identical to the one already used for the Blocked path at lines 493-508, would be needed), (b) touch `check-invariants.sh` Gate 3's mint-call-site allowlist analysis (a new call site invoking the SMTP sink), and (c) most importantly is explicitly ROADMAP-assigned to Phase 17. [The panel found (b) and (c) false — see the superseding note above.]
...
- ~~**Building new Allowed-path sink-invocation wiring for `email.send` to satisfy CONTROL-01:** This is Phase 17/ACCEPT-01's explicit job per ROADMAP.md...~~ **[STRUCK — Round-1 panel ruling.]** This anti-pattern was WRONG: CONTROL-01 is Phase 16's (REQUIREMENTS:95); Phase 17 only composes it. The Gate-3 concern is moot — email_smtp resolves (never mints), so server.rs's existing mint-token allowlist is untouched. Phase 16 BUILDS the wiring (16-01 Task 3) and proves live delivery (16-03). See the superseding note under finding #6 and Open Question 1.
...
| `email.send` sink only invoked from the confirm path | An email.send Allowed-decision dispatch branch is ADDED (16-01 Task 3, mirroring file.create); a trusted send now reaches the SMTP adapter | This phase (Round-1 panel ruling — CONTROL-01 is Phase 16's) | CONTROL-01's "actual send" is PROVEN live in Mailpit (16-03); the single-run DAG composition remains Phase 17's |
...
1. **Does Phase 16's CONTROL-01 need to prove an actual completed SMTP send, or is the Allowed-decision proof sufficient?**
   ...
   - Recommendation [RESOLVED — Round-1 panel ruling, caprun-opus-77]: The original recommendation (defer real delivery to Phase 17) is OVERRULED. Phase 16 BUILDS the email.send Allowed-decision dispatch (16-01 Task 3) and PROVES a real trusted send lands in Mailpit (16-03, under scripts/mailpit-verify.sh) — CONTROL-01 is Phase 16's requirement (:95), not Phase 17's. What REMAINS Phase 17's (ACCEPT-01, "composes... in the SAME run") is the single-process/single-session/single-DAG composition of CONTROL-01 + the hostile block; the current per-call fresh-Uuid+audit.db shape (two DBs) is that composition's gap, not this phase's. The deny-sends-nothing send-level proof is likewise carried to Phase 17 (see 16-03's verification mandate).
