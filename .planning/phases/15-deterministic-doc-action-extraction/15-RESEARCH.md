...
| Transform application (concat, base64-decode) on already-extracted raw fields | Confined Worker | — | `DESIGN-confirm-binding.md` "Post-Transformation Bytes" section: the transform runs before crossing into a mint call; doing it broker-side would put hostile-byte manipulation in the control plane, same violation EXTRACT-01 forbids. Carve-out: `mint_from_derivation`'s broker-side `join(input_literals, '@') == transformed_literal` check is NOT a violation of this row — EXTRACT-01 forbids a PARSER over hostile bytes, and a join+equality comparison of two already-minted/validated literals has no parser surface. Field extraction and any base64-decode stay worker-only/deferred; only the concat-verify equality check runs broker-side. |
